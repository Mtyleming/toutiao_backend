...
async def get_history_list(db: AsyncSession, user_id: int, page: int = 1, page_size: int = 10):
    ...
    query = text(
        "SELECT news.*, history.id AS history_id, history.view_time AS view_time "
        "FROM news "
        "INNER JOIN history ON news.id = history.news_id "
        "WHERE history.user_id = :user_id "
        "ORDER BY history.view_time DESC "
        "LIMIT :offset, :page_size"
    )

    result = await db.execute(query, {"user_id": user_id, "offset": offset, "page_size": page_size})
