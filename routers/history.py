...
async def add_history(data: HistoryAddRequest,
    ...
    return success_response(meg="添加成功", data=result)
...
async def get_history_list(page: int = Query(1, ge=1),
    ...
    dto = HistoryListResponse(list=rows, total=total, hasMore=has_more)

    return success_response(meg="获取历史记录成功", data=dto)
...
async def delete_history(news_id: int,
    ...
    return success_response(meg="删除成功")
...
async def clear_history(user: User = Depends(get_current_user),
    ...
    return success_response(meg="清空成功")
